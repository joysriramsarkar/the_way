...
(function(window) {
  ...
  // State
  const SUPPORTED_LANGS = ['bn', 'en', 'es', 'hi', 'ar', 'pt', 'fr', 'ru'];
  let currentLang = localStorage.getItem('theway_lang') || localStorage.getItem('theway_locale') || 'bn';
  if (!SUPPORTED_LANGS.includes(currentLang)) currentLang = 'bn';
  ...
  function setLanguage(lang) {
    ...
    localStorage.setItem('theway_locale', lang);
    document.documentElement.lang = lang;
    document.documentElement.dir = (['ar', 'fa', 'ur', 'he'].includes(lang) ? 'rtl' : 'ltr');
    if (window.TheWayI18n && typeof window.TheWayI18n.setLocale === 'function') {
      window.TheWayI18n.setLocale(lang);
    }
    renderAllComponents();
        ...
        <div class="ticker-inner">
          ...
          <div class="ticker-actions">
            <button class="lang-selector-btn" id="lang-modal-trigger" onclick="TheWayComponents.openModal('lang-modal')" title="Change Language">
              ${SVGIcons.globe} <span>${({ bn: 'বাংলা', en: 'English', es: 'Español', hi: 'हिन्दी', ar: 'العربية', pt: 'Português', fr: 'Français', ru: 'Русский' })[currentLang] || currentLang}</span> ▾
            </button>
        ...
        <div class="container">
          <div class="nav-strip-inner">
            <div class="nav-sections-scroll" id="nav-sections-scroll">
              ${navLinksHtml}
              <a href="/feed.html" class="nav-sec-link ${currentPath.includes('feed') ? 'active' : ''}" style="color:var(--crimson-primary); font-weight:700;">
                🌍 ${currentLang === 'bn' ? 'ফিড' : 'Feed'}
              </a>
              <a href="/directory.html" class="nav-sec-link ${currentPath.includes('directory') ? 'active' : ''}">
                👥 ${currentLang === 'bn' ? 'ডিরেক্টরি' : 'Directory'}
              </a>
              <a href="/groups.html" class="nav-sec-link ${currentPath.includes('group') ? 'active' : ''}">
                📚 ${currentLang === 'bn' ? 'পাঠচক্র' : 'Groups'}
              </a>
              <a href="/solidarity.html" class="nav-sec-link ${currentPath.includes('solidarity') ? 'active' : ''}" style="color:#f87171; font-weight:700;">
                ✊ ${currentLang === 'bn' ? 'সংহতি' : 'Solidarity'}
              </a>
              <a href="/translations.html" class="nav-sec-link ${currentPath.includes('translations') ? 'active' : ''}" style="color:#38bdf8; font-weight:700;">
                🌐 ${currentLang === 'bn' ? 'অনুবাদ' : 'Translations'}
              </a>
              <a href="/organizations.html" class="nav-sec-link ${currentPath.includes('organizations') ? 'active' : ''}" style="color:#4ade80; font-weight:700;">
                🚩 ${currentLang === 'bn' ? 'সংগঠন' : 'Organizations'}
              </a>
              <a href="/languages.html" class="nav-sec-link ${currentPath.includes('languages') ? 'active' : ''}">
                🗣️ ${currentLang === 'bn' ? 'ভাষা' : 'Languages'}
              </a>
              <a href="/search.html" class="nav-sec-link ${currentPath.includes('search') ? 'active' : ''}" style="color:#e8b84b; font-weight:700;">
                🔍 ${currentLang === 'bn' ? 'অনুসন্ধান' : 'Search'}
              </a>
    ...
    div.innerHTML = `
      ...
      <div class="theway-modal-backdrop" id="lang-modal">
        <div class="modal-window" style="max-width:480px; text-align:center;">
          ...
          <h3 style="font-family:var(--font-bengali-serif); font-size:1.4rem; margin-bottom:1.25rem; display:flex; align-items:center; justify-content:center; gap:0.5rem;">
            ${SVGIcons.globe} <span>ভাষা নির্বাচন করুন / Select Language</span>
          </h3>
          <div style="display:grid; grid-template-columns:1fr 1fr; gap:0.75rem;">
            <button class="btn-quote-action" style="justify-content:center; font-weight:700; ${currentLang === 'bn' ? 'background:var(--crimson-primary); color:#fff; border-color:var(--crimson-primary);' : ''}" onclick="TheWayComponents.setLanguage('bn'); TheWayComponents.closeModal('lang-modal');">
              বাংলা (Bengali)
            </button>
            <button class="btn-quote-action" style="justify-content:center; font-weight:700; ${currentLang === 'en' ? 'background:var(--crimson-primary); color:#fff; border-color:var(--crimson-primary);' : ''}" onclick="TheWayComponents.setLanguage('en'); TheWayComponents.closeModal('lang-modal');">
              English (International)
            </button>
            <button class="btn-quote-action" style="justify-content:center; font-weight:700; ${currentLang === 'es' ? 'background:var(--crimson-primary); color:#fff; border-color:var(--crimson-primary);' : ''}" onclick="TheWayComponents.setLanguage('es'); TheWayComponents.closeModal('lang-modal');">
              Español (Spanish)
            </button>
            <button class="btn-quote-action" style="justify-content:center; font-weight:700; ${currentLang === 'hi' ? 'background:var(--crimson-primary); color:#fff; border-color:var(--crimson-primary);' : ''}" onclick="TheWayComponents.setLanguage('hi'); TheWayComponents.closeModal('lang-modal');">
              हिन्दी (Hindi)
            </button>
            <button class="btn-quote-action" style="justify-content:center; font-weight:700; ${currentLang === 'ar' ? 'background:var(--crimson-primary); color:#fff; border-color:var(--crimson-primary);' : ''}" onclick="TheWayComponents.setLanguage('ar'); TheWayComponents.closeModal('lang-modal');">
              العربية (Arabic)
            </button>
            <button class="btn-quote-action" style="justify-content:center; font-weight:700; ${currentLang === 'pt' ? 'background:var(--crimson-primary); color:#fff; border-color:var(--crimson-primary);' : ''}" onclick="TheWayComponents.setLanguage('pt'); TheWayComponents.closeModal('lang-modal');">
              Português (Portuguese)
            </button>
            <button class="btn-quote-action" style="justify-content:center; font-weight:700; ${currentLang === 'fr' ? 'background:var(--crimson-primary); color:#fff; border-color:var(--crimson-primary);' : ''}" onclick="TheWayComponents.setLanguage('fr'); TheWayComponents.closeModal('lang-modal');">
              Français (French)
            </button>
            <button class="btn-quote-action" style="justify-content:center; font-weight:700; ${currentLang === 'ru' ? 'background:var(--crimson-primary); color:#fff; border-color:var(--crimson-primary);' : ''}" onclick="TheWayComponents.setLanguage('ru'); TheWayComponents.closeModal('lang-modal');">
              Русский (Russian)
            </button>
          </div>
          <div style="margin-top:1.25rem; border-top:1px solid var(--border-color); padding-top:0.75rem;">
            <a href="/languages.html" style="font-size:0.85rem; color:var(--crimson-primary); font-weight:600;" onclick="TheWayComponents.closeModal('lang-modal')">
              🗣️ সকল ভাষার হাব ও অনুবাদ পরিসংখ্যানে যান →
            </a>
          </div>
        </div>
      </div>
