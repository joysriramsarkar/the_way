/**
 * THE WAY (দ্য ওয়ে) — API Configuration & Core Fallback Engine
 * International Socialist Editorial & Movement Portal
 * All initial articles are authored strictly by 'সম্পাদকীয়' / 'অ্যাডমিন প্যানেল' (Editorial Board)
 */

(function(window) {
  'use strict';

  // Foundational revolutionary socialist sections
  ...
  // Inaugural socialist essays & analyses (Authored by 'সম্পাদকীয়' / 'দ্য ওয়ে সম্পাদকীয় পর্ষদ')
  const DEFAULT_ARTICLES = [
    ...
    },
    {
      ...
      author: 'সম্পাদকীয়',
      author_role: 'দ্য ওয়ে সম্পাদকীয় পর্ষদ',
      author_bio: 'চীনা সমাজতান্ত্রিক ইতিহাস ও মাওবাদী দ্বন্দ্বতত্ত্ব পর্যালোচনা।',
    ...
    },
    {
      ...
      author: 'সম্পাদকীয়',
      author_role: 'দ্য ওয়ে সম্পাদকীয় পর্ষদ',
      author_bio: 'লেনিনবাদী পররাষ্ট্রনীতি ও সাম্রাজ্যবাদ বিরোধী আন্দোলন পর্যালোচনা।',
  ...
  window.THE_WAY_CONFIG = {
    ...
    async fetchArticles(sectionSlug) {
      try {
        let url = '/api/articles?action=list';
        if (sectionSlug && sectionSlug !== 'all') {
          url = `/api/articles?action=public&section=${encodeURIComponent(sectionSlug)}`;
        }
    ...
    async fetchArticleBySlug(slug) {
      try {
        const res = await fetch(`/api/articles?action=public-get&slug=${encodeURIComponent(slug)}`);
        if (res.ok) {
          const data = await res.json();
