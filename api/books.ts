/**
 * api/books.ts — Books Library API
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  const { action, slug, id, page } = req.query as { action?: string; slug?: string; id?: string; page?: string };
  ...
  if (req.method === 'POST' && action === 'import') {
    try {
      ...
      if (payload.pages) {
        ...
        const rows = pageEntries.map(([pageNum, pData]: [string, any]) => ({
